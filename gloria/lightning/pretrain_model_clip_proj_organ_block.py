...
class PretrainCLIPProjOrganBModel(LightningModule):
    def __init__(self, cfg):
        super().__init__()

        self.cfg = cfg
        self.save_hyperparameters(self.cfg)
        self.gloria = builder.build_CLIP_proj_organ_block_model(cfg)
    ...
    def shared_step(self, batch, split):
        ...
        img_emb_g, all_organ_img_features, text_emb_g, all_organ_text_features, sents = self.gloria(batch)
        loss = self.gloria.calc_loss(
            img_emb_g, all_organ_img_features, text_emb_g, all_organ_text_features
        )
