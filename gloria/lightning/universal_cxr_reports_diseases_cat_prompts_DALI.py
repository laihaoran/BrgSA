...
class PretrainUniversalCRDCPDALIModel(LightningModule):
    def __init__(self, cfg):
        super().__init__()

        self.cfg = cfg
        self.save_hyperparameters(self.cfg)
        self.gloria = builder.build_universal_CRDCPDALI_model(cfg)
    ...
    def shared_step(self, batch, split):
        ...
        # ipdb.set_trace()
        img_emb_l, img_emb_g, text_emb_l, text_emb_g, sents, r_cls, cls = self.gloria(batch)

        loss, attn_maps = self.gloria.calc_loss(
            img_emb_l, img_emb_g, text_emb_l, text_emb_g, sents, r_cls, batch['r_label'], cls, batch['label'], split
        )
