...
class PretrainUniversalCRDPSModel(LightningModule):
    def __init__(self, cfg):
        super().__init__()

        self.cfg = cfg
        self.save_hyperparameters(self.cfg)
        self.gloria = builder.build_universal_CRDPS_model(cfg)
    ...
    def shared_step(self, batch, split):
        ...
        img_emb_l, img_emb_g, text_emb_l, text_emb_g, sents, cls = self.gloria(batch)
        loss, attn_maps = self.gloria.calc_loss(
            img_emb_l, img_emb_g, text_emb_l, text_emb_g, sents, cls, batch['label'], split
        )
