...
class GoRIADict(nn.Module):
    def __init__(self, cfg):
        super(GoRIADict, self).__init__()
        ...
        self.lamad1_loss_weight = self.cfg.model.gloria.lamad1_loss_weight
    ...
    def calc_loss(self, img_emb_g_recon, img_z, img_emb_g, text_emb_g_recon, text_z, text_emb_g, sents):
        ...
        img_l1_loss =  F.l1_loss(img_z, torch.zeros_like(img_z), reduction='sum')
        text_l1_loss =  F.l1_loss(text_z, torch.zeros_like(text_z), reduction='sum')

        

        # global loss
        g_loss0, g_loss1 = self._calc_global_loss(img_emb_g, text_emb_g)
        g_rec_loss0, g_rec_loss1 = self._calc_global_loss(img_z, text_z)
        ...
        loss += (img_l1_loss + text_l1_loss) * self.lamad1_loss_weight 
    ...
    def forward(self, x):
        ...
        img_emb_g_recon, img_z = self.dictionarylearning(img_emb_g, sparse=True)
        text_emb_g_recon, text_z = self.dictionarylearning(text_emb_g, sparse=True)


        return img_emb_g_recon, img_z, img_emb_g, text_emb_g_recon, text_z, text_emb_g, sents
