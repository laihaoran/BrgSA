...
class UniversalRAM(nn.Module):
    def __init__(self, cfg):
        super(UniversalRAM, self).__init__()
        ...
        # with torch.no_grad():
        #     self.extract = builder.build_ram_extract_module()
        # ipdb.set_trace()
        self.ram = builder.build_ram_module(cfg)
    ...
    # def ram_extract_forward(self, imgs):
    #     with torch.no_grad():
    #         features = self.extract(imgs)
    #     return features
    
    def ram_forward(self, index, feature):
        aug_feature = self.ram(index, feature)
        return aug_feature
    ...
    def forward(self, x):
        ...
        # with torch.no_grad():
        #     ram_features = self.ram_extract_forward(x["ram_imgs"])

        # # ram features
        # text_emb_g = self.ram_forward(ram_features, text_emb_g)

        ## 修正label数据的report
        index = np.where(x['flag'] == 1)
        if np.sum(index) != 0:
            # ipdb.set_trace()
            text_emb_g_partial = self.ram_forward(x['index'][index] , text_emb_g[index])
            text_emb_g[index] = text_emb_g_partial

        # text_emb_g = self.ram_forward(x['index'] , text_emb_g)
    ...
    def process_img(self, paths, device):
        ...
        for p in paths:
            ...
            # ram_img = ram_transform(img)

            all_imgs.append(torch.tensor(img))
            # all_ram_imgs.append(torch.tensor(ram_img))

        all_imgs = torch.stack(all_imgs).to(device)
        # all_ram_imgs = torch.stack(all_ram_imgs).to(device)
