...
class GLoRIADQNWOSAGLMLPAUG(nn.Module):
    def __init__(self, cfg):
        super(GLoRIADQNWOSAGLMLPAUG, self).__init__()
    ...
    def forward(self, x):
        ...
        chunks_local = torch.chunk(text_emb_l, img_emb_g.size(0), dim=0)

        # 从每个块中提取第一个8x768的张量
        extracted_tensors = [chunk[0, :] for chunk in chunks_local]

        # 将这些张量堆叠起来形成一个256x grid * 768的张量
        text_emb_l = torch.stack(extracted_tensors, dim=0)
        
        
        # extract global features
        chunks_global = torch.chunk(text_emb_g, img_emb_g.size(0), dim=0)
                       
           # 从每个块中提取第一个8x768的张量
        extracted_tensors = [chunk[0, :] for chunk in chunks_global]

        # 将这些张量堆叠起来形成一个256x grid * 768的张量
        text_emb_g = torch.stack(extracted_tensors, dim=0)
        

      # 对每个 8x768 的块进行 repeat 操作，使其成为 8x(32x768) 的张量
        repeated_chunks = [chunk.repeat(img_emb_g.size(0) // 8, 1) for chunk in chunks_global]

        # 将这些扩展后的块连接起来
        text_emb_g_repeat = torch.stack(repeated_chunks, dim=0)
        ...
        i2t_cls = self.fusion_module(torch.cat([img_emb_g.unsqueeze(1) , img_emb_l_], dim=1) , text_emb_g_repeat, inside_repeat=False).squeeze(-1)
