...
class CLIPProjOrganB(nn.Module):
    def __init__(self, cfg):
        super(CLIPProjOrganB, self).__init__()
    ...
    def calc_loss(self, img_emb_g, all_organ_img_features, text_emb_g, all_organ_text_features):
        ...
        if len(all_organ_img_features) > 0:
            ...
            for organ_img_features, organ_text_features in zip(all_organ_img_features, all_organ_text_features):
                organ_img_features = F.normalize(organ_img_features, dim=-1)
                g_loss0_organ_, g_loss1_organ_ = self._calc_global_loss(organ_img_features, organ_text_features)
    ...
    def forward(self, x):
        ...
        img_emb_g = self.organ_attention(img_emb_l)
        ...
        # text encoder branch for organ_caps
        ...
        if len(organ_groups) > 0:
            ...
            for organ_group, organ_name in organ_groups:
                ...
                for sample_idx, organ_idx in organ_group:
                    ...
                    organ_token_embeddings = img_emb_l[sample_idx][organ_token_indices]
                    organ_img_feature = self.organ_attention(organ_token_embeddings.unsqueeze(0)).squeeze(0)
                    
                    # 提取对应文本特征
                    organ_text_feature = shot_text_emb_g[sample_idx * self.cfg.data.text.merge_num  + organ_idx]
                    
                    # 将图像和文本特征加入配对列表
                    organ_img_features.append(organ_img_feature)
        ...
        return img_emb_g, all_organ_img_features, text_emb_g, all_organ_text_features, sents
