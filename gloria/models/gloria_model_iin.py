...
class GLoRIAIIN(nn.Module):
    def __init__(self, cfg):
        super(GLoRIAIIN, self).__init__()

        self.cfg = cfg
        self.text_encoder = builder.build_text_model(cfg)
        self.img_encoder = builder.build_img_model(cfg)
        self.fusion_module = builder.build_iin_module(cfg)
    ...
    def forward(self, x):
        ...
        i2t_cls = self.fusion_module(img_emb_g, text_emb_g).squeeze(-1)
        t2i_cls = self.fusion_module(text_emb_g, img_emb_g).squeeze(-1)
        # 
