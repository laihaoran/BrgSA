...
class CLIPProjDictOrganOneLarge(nn.Module):
    def __init__(self, cfg):
        super(CLIPProjDictOrganOneLarge, self).__init__()
        ...
        self.global_token_indices = np.load(self.cfg.data.image.global_token_indices)
    ...
    def calc_loss(self, img_emb_g, img_emb_g_recon, text_emb_g, text_emb_g_recon,  sents):
        ...
        loss += (g_rec_loss0 + g_rec_loss1) * self.global_loss_weight 
    ...
    def forward(self, x):
        ...
        image_token = self.img_encoder.model.TokenizerImage(x["imgs"])
        cls_token, local_tokens = image_token[:, 0:1], image_token[:, 1:]
        ...
        total_choise = len(organ_groups) + 1
        choise = torch.randint(0, total_choise, (1,)).item()

        if choise == total_choise - 1:
            # calculate the global image token
            global_image_token = torch.cat([cls_token, local_tokens[:, self.global_token_indices]], dim=1)
            img_emb_l, img_emb_g = self.img_encoder.model.self_attention_token(global_image_token)
            img_emb_l, img_emb_g = self.img_encoder.generate_embeddings(
                img_emb_g, img_emb_l
            )
            ...
            return img_emb_g, img_emb_g_recon, text_emb_g, text_emb_g_recon, sents    
        else:
            # text encoder branch for shot_caps
            shot_text_emb_l, shot_text_emb_g, sents = self.text_encoder_forward(
            ...
            for sample_idx, organ_idx in organ_group:
                ...
                organ_token_embeddings = local_tokens[sample_idx][organ_token_indices]
                organ_token_embeddings = torch.cat([cls_token[sample_idx], organ_token_embeddings], dim=0)
                organ_img_feature_g, organ_img_feature_l = self.img_encoder.model.self_attention_token(organ_token_embeddings.unsqueeze(0))
                organ_img_feature_g, organ_img_feature_l = self.img_encoder.generate_embeddings(
                organ_img_feature_g, organ_img_feature_l)
            ...
            all_organ_img_features = torch.stack(organ_img_features)
            all_organ_text_features = torch.stack(organ_text_features)
            all_recon_organ_img_features = self.dictionarylearning(torch.stack(organ_img_features)) 
            all_recon_organ_text_features = self.dictionarylearning(torch.stack(organ_text_features))

             # Example of returning the pairs for contrastive learning
            return all_organ_img_features, all_recon_organ_img_features, all_organ_text_features, all_recon_organ_text_features, sents
