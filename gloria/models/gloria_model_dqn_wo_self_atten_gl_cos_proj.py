...
class GLoRIADQNWOSAGLWOHProjCOS(nn.Module):
    def __init__(self, cfg):
        super(GLoRIADQNWOSAGLWOHProjCOS, self).__init__()

        self.cfg = cfg
        self.text_encoder = builder.build_text_model(cfg)
        self.img_encoder = builder.build_img_model(cfg)
        self.fusion_module = builder.build_dqn_wo_self_atten_wo_head_module(cfg)

        self.local_loss = loss.gloria_loss.local_loss
        self.global_loss = loss.gloria_loss.global_loss_for_ca
    ...
    def calc_loss(self, img_emb_l, img_emb_g, text_emb_l, text_emb_g, sents, proj_img, proj_text):

        # l_loss0, l_loss1, attn_maps = self._calc_local_loss(
        #     img_emb_l, text_emb_l, sents
        # )

        # img_emb_g = img_emb_g.unsqueeze(1).repeat(1, text_emb_g.size(0), 1) 
        # text_emb_g = text_emb_g.unsqueeze(1).repeat(1, img_emb_g.size(0), 1)

        g_loss0, g_loss1 = self._calc_global_loss(proj_img.transpose(1, 0), proj_text)

        # g_loss0, g_loss1 = self._calc_global_loss(proj_img.transpose(1, 0), text_emb_g)
        # g_loss2, g_loss3 = self._calc_global_loss(img_emb_g, proj_text.transpose(1, 0))
    ...
    def forward(self, x):
        ...
        proj_img = self.fusion_module(torch.cat([img_emb_g.unsqueeze(1) , img_emb_l_], dim=1) , text_emb_g).squeeze(-1)
        ...
        # 
        proj_text = self.fusion_module(torch.cat([text_emb_g.unsqueeze(1) , text_emb_l_], dim=1) , img_emb_g).squeeze(-1)

        # 
        return img_emb_l, img_emb_g, text_emb_l, text_emb_g, sents, proj_img, proj_text
