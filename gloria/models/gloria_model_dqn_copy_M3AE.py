...
class GLoRIADQNM3AE(nn.Module):
    def __init__(self, cfg):
        super(GLoRIADQNM3AE, self).__init__()

        self.cfg = cfg
        self.text_encoder = builder.build_text_model(cfg)
        self.img_encoder = builder.build_img_model(cfg)
        self.fusion_module = builder.build_dqn_M3AE_module(cfg)
    ...
    def forward(self, x):
        ...
        batch_size = img_emb_g.size(0)

        img_emb_l_ = img_emb_l.view(img_emb_l.size(0), img_emb_l.size(1), -1).permute(0, 2, 1) # [512, 768, 14, 14] -> [512, 768, 196]
        text_emb_l_ = text_emb_l.view(text_emb_l.size(0), text_emb_l.size(1), -1).permute(0, 2, 1)

        img_emb_g_ = img_emb_g.unsqueeze(1).repeat(1, batch_size, 1) # (query_num,batch_size,dim)
        text_emb_g_ = text_emb_g.unsqueeze(1).repeat(1, batch_size, 1) # (query_num,batch_size,dim)

        batch = [ img_emb_g_, text_emb_g_, img_emb_l_, text_emb_l_]
        i2t_cls, t2i_cls = self.fusion_module(batch, return_atten=False)
        ...
        # i2t_cls = self.fusion_module(img_emb_l_, text_emb_g)
