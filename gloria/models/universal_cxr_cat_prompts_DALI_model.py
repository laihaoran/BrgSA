...
class UniversalCXRCPDALI(nn.Module):
    def __init__(self, cfg):
        super(UniversalCXRCPDALI, self).__init__()

        self.cfg = cfg
        self.text_encoder = builder.build_text_model(cfg)
        self.img_encoder = builder.build_img_model(cfg)
        self.query = builder.build_query(cfg)
        self.report_query = builder.build_report_query(cfg)
        self.prompts = builder.build_prompts(cfg)
        self.number_prompts = len(self.prompts)
        ...
        self.ce_loss = loss.MaskBCE_loss.MaskBCELoss()
        ...
        self.ce_loss_weight = self.cfg.model.gloria.ce_loss_weight
    ...
    def _calc_ce_loss(self, cls, label):
        loss = self.ce_loss(cls, label.type(cls.dtype))
        return loss

    
    def calc_loss(self, img_emb_l, img_emb_g, text_emb_l, text_emb_g, sents, r_cls, r_label, cls, label, split='train'):
        ...
        ce_loss = self._calc_ce_loss(cls, label)

        r_ce_loss = self._calc_ce_loss(r_cls, r_label)
        ...
        if split == 'train':
            ...
            loss += r_ce_loss * self.ce_loss_weight
    ...
    def forward(self, x):
        ...
        # print(f"Time taken for image encoder forward pass: {time.time() - s}")
        ...
        print(f"Time taken for text encoder forward pass: {time.time() - s}")

        # s = time.time()

        # img enocder branch for label images
        label_img_emb_l, label_img_emb_g = self.image_encoder_forward(x["label_imgs"])

        # print(f"Time taken for another image enocder forward pass: {time.time() - s}")

        # s = time.time()
        # query generate branch
        device = label_img_emb_g.device 

        # random extract prompts
        random_prompts = random.randint(0, self.number_prompts - 1)
        prompts = self.prompts[random_prompts]

        # print(f"Time taken for random prompts: {time.time() - s}")
        # s = time.time()
        query = [prompts.format(q) for q in self.query]
        report_query = [prompts.format(q) for q in self.report_query]

        # print(f"Time taken for query prompts: {time.time() - s}")
        ...
        process_query = self.process_text(query, device)
        process_report_query = self.process_text(report_query, device)

        s = time.time()
        query_emb_l, query_emb_g, _ = self.text_encoder_forward(
            process_query["caption_ids"], process_query["attention_mask"], process_query["token_type_ids"]
        )
        print(f"Time taken for query embedding: {time.time() - s}")
        
        s = time.time()
        report_query_emb_l, report_query_emb_g, _ = self.text_encoder_forward(
            process_report_query["caption_ids"], process_report_query["attention_mask"], process_report_query["token_type_ids"]
        )

        print(f"Time taken for report query embedding: {time.time() - s}")
        ...
        s = time.time()
        caption_ids = torch.cat([x['caption_ids'],  process_query["caption_ids"], process_report_query["caption_ids"]], dim=0)
        attention_mask = torch.cat([x['attention_mask'],  process_query["attention_mask"], process_report_query["attention_mask"]], dim=0)
        token_type_ids = torch.cat([x['token_type_ids'],  process_query["token_type_ids"], process_report_query["token_type_ids"]], dim=0)
        _, _, _ = self.text_encoder_forward(caption_ids, attention_mask, token_type_ids)
        print(f"Time taken for test embedding: {time.time() - s}")

        #entry report query result
        B = img_emb_g.size(0)

        report_query_emb_g = report_query_emb_g.unsqueeze(1).repeat(1, B, 1)

        img_emb_l_ = img_emb_l.view(img_emb_l.size(0), img_emb_l.size(1), -1) 

        img_emb_l_ = img_emb_l_.permute(2, 0, 1) #patch_num b dim
        # 
        r_cls = self.fusion_module(report_query_emb_g, img_emb_l_).squeeze(-1)

        # print(f"Time taken for report query fusion: {time.time() - s}")
        ...
        # 
        cls = self.fusion_module(query_emb_g, label_img_emb_l).squeeze(-1)

        # print(f"Time taken for query fusion: {time.time() - s}")

        return img_emb_l, img_emb_g, text_emb_l, text_emb_g, sents, r_cls, cls
    ...
    def process_img(self, paths, device):
        ...
        load_dataset = dataset(self.cfg, paths, transform)
        loader = data.DataLoader(load_dataset, batch_size=len(paths), shuffle=False, num_workers=4, drop_last=False)
        all_imgs = next(iter(loader)).to(device)
...
class dataset(data.Dataset):
    def __init__(self, cfg, paths, transformer) -> None:
        self.cfg = cfg
        self.paths = paths
    ...
    def __getitem__(self, index):
        x = cv2.imread(str(self.paths[index]), 0)
        # tranform images
        x = self._resize_img(x, self.cfg.data.image.imsize)
        img = Image.fromarray(x).convert("RGB")
