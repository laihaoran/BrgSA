...
class GLoRIAIINATTLG(nn.Module):
    def __init__(self, cfg):
        super(GLoRIAIINATTLG, self).__init__()

        self.cfg = cfg
        self.text_encoder = builder.build_text_model(cfg)
        self.img_encoder = builder.build_img_model(cfg)
        self.fusion_module = builder.build_iin_share_module(cfg)
    ...
    def calc_loss(self, img_emb_l, img_emb_g, text_emb_l, text_emb_g, sents, i2t_cls, t2i_cls, i2t_l_cls, t2i_l_cls, i2t_g_cls,t2i_g_cls):
        ...
        ce_loss2 = self._calc_ce_loss(i2t_g_cls)
        ce_loss3 = self._calc_ce_loss(t2i_g_cls)
        ce_loss4 = self._calc_ce_loss(i2t_l_cls)
        ce_loss5 = self._calc_ce_loss(t2i_l_cls)
        loss += ce_loss0 * self.ce_loss_weight
        loss += ce_loss1 * self.ce_loss_weight
        loss += ce_loss2 * self.ce_loss_weight
        loss += ce_loss3 * self.ce_loss_weight
        loss += ce_loss4 * self.ce_loss_weight
        loss += ce_loss5 * self.ce_loss_weight
    ...
    def forward(self, x):
        ...
        i2t_cls, img_atten = self.fusion_module(img_emb_l_, text_emb_g,mode='local', return_atten=True)
        # 
        i2t_cls = i2t_cls.squeeze(-1) # image to text
        ...
        # 
        t2i_cls, text_atten = self.fusion_module(text_emb_l_, img_emb_g,mode='local', return_atten=True)

        t2i_cls = t2i_cls.squeeze(-1) # text to image


        img_atten = (img_atten[-4] +img_atten[-3]+img_atten[-2]+img_atten[-1])/4 # obtain the heat map of patch-level attention
        text_atten = (text_atten[-4] +text_atten[-3]+text_atten[-2]+text_atten[-1])/4  # obtain the heat map of word-level attention

        # 计算 img_atten 的最大索引
        img_atten_max_indices = img_atten.argmax(2)  # 256 x 256
        text_atten_max_indices = text_atten.argmax(2)  # 256 x 256

        # 创建一个索引张量以选择 img_emb_l_ 中的图像补丁
        row_indices = torch.arange(Batch_size)
        img_patch_indices = img_atten_max_indices[row_indices, row_indices]
        text_word_indices = text_atten_max_indices[row_indices, row_indices]

        # 使用索引张量获取所需的图像补丁
        hight_patch = img_emb_l_[row_indices, img_patch_indices]
        hight_word = text_emb_l_[row_indices, text_word_indices]
        # 

        i2t_l_cls = self.fusion_module(hight_patch, hight_word, mode='global').squeeze(-1)
        t2i_l_cls = self.fusion_module(hight_word, hight_patch, mode='global').squeeze(-1)

        i2t_g_cls = self.fusion_module(img_emb_g, text_emb_g, mode='global').squeeze(-1)
        t2i_g_cls = self.fusion_module(text_emb_g, img_emb_g, mode='global').squeeze(-1)

        return img_emb_l, img_emb_g, text_emb_l, text_emb_g, sents, i2t_cls, t2i_cls, i2t_l_cls, t2i_l_cls, i2t_g_cls,t2i_g_cls
