...
class GLoRIADQNATT(nn.Module):
    def __init__(self, cfg):
        super(GLoRIADQNATT, self).__init__()

        self.cfg = cfg
        self.text_encoder = builder.build_text_model(cfg)
        self.img_encoder = builder.build_img_model(cfg)
        self.fusion_module = builder.build_dqn_module(cfg)
    ...
    def calc_loss(self, img_emb_l, img_emb_g, text_emb_l, text_emb_g, sents, i2t_cls, t2i_cls, i2ht_cls, t2hp_cls):
        ...
        ce_loss2 = self._calc_ce_loss(i2ht_cls)
        ce_loss3 = self._calc_ce_loss(t2hp_cls)
    ...
    def forward(self, x):
        ...
        i2t_cls, img_atten = self.fusion_module(img_emb_l_, text_emb_g, return_atten=True)
        # 
        ...
        # 
        t2i_cls, text_atten = self.fusion_module(text_emb_l_, img_emb_g, return_atten=True)
        ...
        i2ht_cls = self.fusion_module(img_emb_l_, hight_word).squeeze(-1)
        t2hp_cls = self.fusion_module(text_emb_l_, hight_patch).squeeze(-1)

        # 
        return img_emb_l, img_emb_g, text_emb_l, text_emb_g, sents, i2t_cls, t2i_cls, i2ht_cls, t2hp_cls
