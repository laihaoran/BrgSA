...
class CLIPProjDictOrganCLS(nn.Module):
    def __init__(self, cfg):
        super(CLIPProjDictOrganCLS, self).__init__()

        self.cfg = cfg
        self.text_encoder = builder.build_text_proj_model(cfg)
        self.img_encoder = builder.build_img_model(cfg)
        self.organ_cls_token = builder.build_organ_cls_token_model(cfg)
    ...
    def forward(self, x):
        ...
        if len(organ_groups) > 0:
            ...
            for organ_group, organ_name in organ_groups:
                organ_img_features = []
                organ_text_features = []
                organ_cls_token = self.organ_cls_token.cls_tokens[organ_name][0]
                # 提取该器官的图像和文本特征
