...
class GLoRIADQNWOSAGLP(nn.Module):
    def __init__(self, cfg):
        super(GLoRIADQNWOSAGLP, self).__init__()

        self.cfg = cfg
        self.text_encoder = builder.build_text_model(cfg)
        self.img_encoder = builder.build_img_model(cfg)
        self.fusion_module = builder.build_dqn_wo_self_atten_module(cfg)

        # add projection layer from M3AE
        self.multi_modal_language_proj = nn.Linear(cfg.model.fusion.d_model, cfg.model.fusion.d_model)
        self.multi_modal_vision_proj = nn.Linear(cfg.model.fusion.d_model, cfg.model.fusion.d_model)
    ...
    def _load_pretrained_model(self, path_to_pretrained_model):
        ...
        try:
            ...
            print(f"load keys: {pretrained_weights.keys()}")
    ...
    def forward(self, x):
        ...
        img_emb_l_ = self.multi_modal_vision_proj(img_emb_l_)
        img_emb_g = self.multi_modal_vision_proj(img_emb_g)
        text_emb_l_ = self.multi_modal_language_proj(text_emb_l_)
        text_emb_g = self.multi_modal_language_proj(text_emb_g)
