...
class CLIPProjGL(nn.Module):
    def __init__(self, cfg):
        super(CLIPProjGL, self).__init__()

        self.cfg = cfg
        self.text_encoder = builder.build_text_proj_attention_model(cfg)
        ...
        self.adaptive_local_loss = loss.local_loss.LocalLossCalculator(num_blocks=22, num_phrases=24, top_k=5, tau_local=0.07)
    ...
    def text_encoder_forward(self, caption_ids, attention_mask, token_type_ids):
        text_emb_l, text_emb_g, sents, attention_map = self.text_encoder(
            caption_ids, attention_mask, token_type_ids
        )
        return text_emb_l, text_emb_g, sents, attention_map

    def image_encoder_forward(self, imgs):
        img_feat_g, img_emb_l, attention_map = self.img_encoder(imgs, get_atten=True)
        ...
        return img_emb_l, img_emb_g, attention_map
    ...
    def calc_loss(self, img_emb_l, img_emb_g, img_attention_map, text_emb_l, text_emb_g, text_attention_map, sents):
        ...
        local_loss = self.adaptive_local_loss(img_emb_l, img_attention_map, text_emb_l, text_attention_map)
        ...
        loss += local_loss * self.local_loss_weight
    ...
    def forward(self, x):
        ...
        # img encoder branch
        img_emb_l, img_emb_g, img_attention_map = self.image_encoder_forward(x["imgs"])
    
        # text encorder branch
        text_emb_l, text_emb_g, sents, text_attention_map = self.text_encoder_forward(
            x["caption_ids"], x["attention_mask"], x["token_type_ids"]
        )
        # 

        return img_emb_l, img_emb_g, img_attention_map, text_emb_l.permute(0, 2, 1), text_emb_g, text_attention_map, sents
