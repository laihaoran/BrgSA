...
class CLIPProjDictMSE(nn.Module):
    def __init__(self, cfg):
        super(CLIPProjDictMSE, self).__init__()
    ...
    def calc_loss(self, img_emb_g_recon, img_emb_g, z):
        # reconstruction loss
        ...
        # l2_reg =  torch.norm(z, p=2) ** 2
        # weighted loss
        loss = 0

        # loss += (g_loss0 + g_loss1) * self.global_loss_weight
        loss += ( img_recon_loss ) * self.global_loss_weight
        loss += l1_reg * 0.01
        # loss += l2_reg * 0.01
        return loss
    ...
    def forward(self, x):
        ...
        img_emb_g_recon, z = self.dictionarylearning(img_emb_g, sparse=True)

        return img_emb_g_recon, img_emb_g, z
