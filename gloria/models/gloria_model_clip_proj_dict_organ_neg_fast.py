...
class CLIPProjDictOrganNegFast(nn.Module):
    def __init__(self, cfg):
        super(CLIPProjDictOrganNegFast, self).__init__()
        ...
        self.organ_loss = loss.gloria_loss.global_organ_loss
    ...
    def _calc_organ_loss(self, organ_img_features, organ_text_features, organ_label):
        g_loss0, g_loss1 = self.organ_loss(organ_img_features, organ_text_features, organ_label, temp3=self.temp3)
        return g_loss0, g_loss1

    def calc_loss(self, img_emb_g, img_emb_g_recon, all_organ_img_features, all_recon_organ_img_features, text_emb_g, text_emb_g_recon,  all_organ_text_features, all_recon_organ_text_features, all_organ_label):
        ...
        if len(all_organ_img_features) > 0:
            ...
            for organ_img_features, organ_img_features_recon, organ_text_features, organ_text_features_recon, organ_label in zip(all_organ_img_features, all_recon_organ_img_features, all_organ_text_features, all_recon_organ_text_features, all_organ_label):
                ...
                g_loss0_organ_, g_loss1_organ_ = self._calc_organ_loss(F.normalize(organ_img_features, dim=-1), F.normalize(organ_text_features, dim=-1), organ_label)
                g_loss0_organ += g_loss0_organ_
                g_loss1_organ += g_loss1_organ_

                g_rec_loss0_organ_, g_rec_loss1_organ_ = self._calc_organ_loss(F.normalize(organ_img_features_recon, dim=-1), F.normalize(organ_text_features_recon, dim=-1), organ_label)
        ...
        if organ_count > 0:
            loss += (g_loss0_organ + g_loss1_organ) / organ_count * self.global_loss_weight
            loss += (organ_img_recon_loss + organ_text_recon_loss) / organ_count * self.mse_loss_weight
            loss += (g_rec_loss0_organ + g_rec_loss1_organ) / organ_count * self.global_loss_weight
        return loss
    ...
    def forward(self, x):
        ...
        disease_organ = x['disease_organ']
    
        organ_groups = self.get_organ_groups_with_indices(organ, shot_organ_indices)
        organ_disease_label = self.generate_organ_labels_fast(organ_groups, disease_organ)

        all_organ_labels = []
        all_organ_img_features = []
        all_organ_text_features = []
        
        organ_feature_lengths = []  # To track length for each organ's features


        for organ_group, organ_name in organ_groups:
            if torch.sum(organ_disease_label[organ_name]) != 0:
                all_organ_labels.append(organ_disease_label[organ_name])

                organ_img_features = []
                organ_text_features = []

                for sample_idx, organ_idx in organ_group:
                    # Extract organ token embeddings from hidden states
                    organ_token_indices = shot_organ_indices[sample_idx][organ_idx]
                    organ_token_embeddings = torch.cat(
                        [hidden_global[sample_idx], hidden_local[sample_idx][organ_token_indices]], dim=0
                    )
                    organ_img_features.append(organ_token_embeddings)

                    # Corresponding text feature extraction
                    organ_text_features.append(shot_text_emb_g[sample_idx * self.cfg.data.text.organ_number + organ_idx])

                # Append features and record lengths
                all_organ_img_features.extend(organ_img_features)
                all_organ_text_features.extend(organ_text_features)
                organ_feature_lengths.append(len(organ_img_features))

        # Use pad_sequence for padding all organ_img_features to the same length
        organ_img_feature_batch_tensor = pad_sequence(all_organ_img_features, batch_first=True)
        organ_text_feature_batch_tensor = torch.stack(all_organ_text_features)
        organ_attention_mask = (organ_img_feature_batch_tensor != 0).any(dim=-1).int()

        # Forward pass through the image encoder
        organ_img_features = self.img_encoder.model.blocks[11](
            organ_img_feature_batch_tensor, mask=organ_attention_mask
        )
        organ_img_features = self.img_encoder.model.norm(organ_img_features)

        organ_img_feature_g, organ_img_feature_l = self.img_encoder.generate_embeddings(
            organ_img_features[:, 0], organ_img_features[:, 1:]
        )

        # Concatenate original and organ-based features for a single dictionarylearning call
        combined_features = torch.cat([img_emb_g, text_emb_g, organ_img_feature_g, organ_text_feature_batch_tensor], dim=0)
        
        # Single pass through dictionarylearning
        combined_features_recon = self.dictionarylearning(combined_features)

        # Split the reconstructed features back into original and organ parts
        (
            img_emb_g_recon, 
            text_emb_g_recon, 
            organ_img_feature_g_recon, 
            organ_text_feature_recon
        ) = torch.split(
            combined_features_recon, 
            [img_emb_g.size(0), text_emb_g.size(0), organ_img_feature_g.size(0), organ_text_feature_batch_tensor.size(0)], 
            dim=0
        )
        
        # Reshape organ_img_feature_g, organ_img_feature_g_recon, organ_text_feature_recon back to list format
        split_indices = torch.cumsum(torch.tensor(organ_feature_lengths), dim=0)[:-1]

        all_organ_img_features = torch.split(organ_img_feature_g, organ_feature_lengths)
        all_organ_img_features_recon = torch.split(organ_img_feature_g_recon, organ_feature_lengths)
        all_organ_text_features = torch.split(organ_text_feature_batch_tensor, organ_feature_lengths)
        all_organ_text_features_recon = torch.split(organ_text_feature_recon, organ_feature_lengths)

        # Example of returning the pairs for contrastive learning
        return img_emb_g, img_emb_g_recon, all_organ_img_features, all_organ_img_features_recon, text_emb_g, text_emb_g_recon,  all_organ_text_features, all_organ_text_features_recon, all_organ_labels, sents

    def generate_organ_labels_fast(self, organ_groups, organ_disease):

        # 将 organ_disease 转换为字典，便于快速查找
        organ_disease_dict = [{org: 1 for org in organs} for organs in organ_disease]
        
        # 初始化每个器官组的标签字典
        organ_labels = {}

        for indices, organ_name in organ_groups:
            # 动态调整器官组的大小
            group_size = len(indices)
            
            # 创建标签张量
            organ_label = torch.zeros(group_size, dtype=torch.long)
            
            # 向量化查找：通过列表解析和字典快速判断器官是否存在
            organ_label = torch.tensor([
                1 if organ_disease_dict[img_idx].get(organ_name, 0) == 1 else 0
                for img_idx, _ in indices
            ], dtype=torch.long)

            # 将生成的标签添加到字典
            organ_labels[organ_name] = organ_label
