...
class GLoRIAIINSWOSA(nn.Module):
    def __init__(self, cfg):
        super(GLoRIAIINSWOSA, self).__init__()

        self.cfg = cfg
        self.text_encoder = builder.build_text_model(cfg)
        self.img_encoder = builder.build_img_model(cfg)
        self.fusion_module = builder.build_iin_share_wo_self_atten_module(cfg)
    ...
    def forward(self, x):
        ...
        # 
        i2t_cls = self.fusion_module(img_emb_l_, text_emb_g,  mode='local').squeeze(-1)
        ...
        # 
        t2i_cls = self.fusion_module(text_emb_l_, img_emb_g, mode='local').squeeze(-1)
        ...
        # 
        return img_emb_l, img_emb_g, text_emb_l, text_emb_g, sents, i2t_cls, t2i_cls, i2t_g_cls, t2i_g_cls
