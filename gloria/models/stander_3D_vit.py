...
class MaskAttention(Attention):
    ...
    def forward(self, x, mask=None):
        ...
        # 扩展 mask 形状以匹配 attn 的形状
        if mask is not None:
            mask = mask.unsqueeze(1).unsqueeze(2)  # 变为 (B, 1, 1, N)
            mask = mask.expand(-1, self.num_heads, N, -1)  # 变为 (B, num_heads, N, N)
...
class CustomAttention(Attention):
    ...
    def forward(self, x):
        ...
        if self.fused_attn:
            ...
            attn = None  # `scaled_dot_product_attention` does not return attention weights
        else:
            q = q * self.scale
...
class CustomViT3D(ViT3D):
    ...
    def forward(self, x, get_local=True):
        ...
        last_attn_weights = None  # 用于保存最后一层的注意力权重
        for blk in self.blocks:
            x, attn_weights = blk(x)  # 获取每一层的注意力权重
            last_attn_weights = attn_weights  # 保存最后一层的注意力权重
        
        # 去除第一个 token（class token）的注意力
        if last_attn_weights is not None:
            # 去掉第一个位置 (class token) 的注意力
            last_attn_weights = last_attn_weights[:, :, 1:, 1:].mean(dim=1)  # [batch_size, num_heads, num_patches, num_patches]
        ...
        return x[:, 0], x[:, 1:], last_attn_weights  # 返回注意力权重
...
def make_3d_model(pretrained=True):
    ...
    if pretrained:
        model = load_weight(model, './Pretrain_model/M3AE_Vision_Encoder/VIT_B16_3D_M3AE.pth')
