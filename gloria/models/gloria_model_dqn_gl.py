...
class GLoRIADQNGL(nn.Module):
    def __init__(self, cfg):
        super(GLoRIADQNGL, self).__init__()
    ...
    def calc_loss(self, img_emb_l, img_emb_g, text_emb_l, text_emb_g, sents, il_2_tg_cls, tl_2_ig_cls, split):
        ...
        # # weighted loss
        loss = 0
        ce_loss0 = self._calc_ce_loss(il_2_tg_cls)
        ce_loss1 = self._calc_ce_loss(tl_2_ig_cls)
    ...
    def forward(self, x):
        ...
        il_2_tg_cls = self.fusion_module(torch.cat([img_emb_g.unsqueeze(1) , img_emb_l_], dim=1), text_emb_g).squeeze(-1)
        ...
        # 
        tl_2_ig_cls = self.fusion_module(torch.cat([text_emb_g.unsqueeze(1) , text_emb_l_], dim=1), img_emb_g).squeeze(-1)



        # 
        return img_emb_l, img_emb_g, text_emb_l, text_emb_g, sents, il_2_tg_cls, tl_2_ig_cls
