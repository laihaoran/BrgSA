...
class CLIPProjDictOrganNeg(nn.Module):
    def __init__(self, cfg):
        super(CLIPProjDictOrganNeg, self).__init__()
    ...
    def forward(self, x):
        ...
        if len(organ_groups) > 0:
            ...
            for organ_group, organ_name in organ_groups:
                ...
                if torch.sum(organ_disease_label[organ_name]) != 0:
                    all_organ_label.append(organ_disease_label[organ_name])
                    # 提取该器官的图像和文本特征
                    for sample_idx, organ_idx in organ_group:
                        # 提取图像特征
                        organ_token_indices = shot_organ_indices[sample_idx][organ_idx]
        ...
        return img_emb_g, img_emb_g_recon, all_organ_img_features, all_recon_organ_img_features, text_emb_g, text_emb_g_recon,  all_organ_text_features, all_recon_organ_text_features, all_organ_label, sents
