...
class CLIPProjMem(nn.Module):
    def __init__(self, cfg):
        super(CLIPProjMem, self).__init__()

        self.cfg = cfg
        self.text_encoder = builder.build_text_proj_model(cfg)  #close proj
        self.img_encoder = builder.build_img_model(cfg)
        self.memory_bank = builder.build_memory_model_global(cfg)
        self.text_recifier = builder.build_Refiner_model_global(self.memory_bank)
    ...
    def calc_loss(self,  img_emb_g, text_emb_g, sents):
    ...
    def forward(self, x, split):
        ...
        # 
         ...
         # 1. 使用 Memory Bank 修正文本特征
        text_emb_g_ = self.text_recifier.refine(text_emb_g, alpha=0.1, temperature=0.07)

        if split == "train":
            # 2. 更新 Memory Bank（使用图像特征）
            self.memory_bank.update(img_emb_g)
            self.memory_bank.update(text_emb_g)
        return img_emb_g, text_emb_g_, sents
