...
class GLoRIADQNRM(nn.Module):
    def __init__(self, cfg):
        super(GLoRIADQNRM, self).__init__()
    ...
    def forward(self, x, split="train"):
        ...
        img_prob_for_masking = random.random()
        text_prob_for_masking = random.random()

        if  img_prob_for_masking < 0.5 and split == "train":
            img_emb_l_ = self.random_masking(img_emb_l_, img_prob_for_masking)
        img_emb_l_ = self.random_masking(img_emb_l_, img_prob_for_masking)
        ...
        # 
        if  text_prob_for_masking < 0.5 and split == "train":
            text_emb_l_ = self.random_masking(text_emb_l_, text_prob_for_masking)
        text_emb_l_ = self.random_masking(text_emb_l_, img_prob_for_masking)
    ...
    def random_masking(self, x, mask_ratio):
        ...
        # pos_embed = self.vision_encoder.pos_embed.to(x)
