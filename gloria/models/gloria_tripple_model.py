...
class GLoRIATripple(nn.Module):
    def __init__(self, cfg):
        super(GLoRIATripple, self).__init__()

        self.cfg = cfg
        self.text_encoder = builder.build_text_model(cfg)
        self.img_encoder = builder.build_img_model(cfg)
        self.gpt_decoder = builder.build_gpt_model(cfg)
    ...
    def gpt_decoder_forward(self, embedding):
        gpt_emb_l, gpt_emb_g = self.gpt_decoder(
            embedding
        )
        return gpt_emb_l, gpt_emb_g
    ...
    def _calc_local_loss(self, img_emb_l, text_emb_l, sents, is_GPT=False):
        if is_GPT:
            cap_lens = sents
        else:
            cap_lens = [
    ...
    def calc_loss(self, img_emb_l, img_emb_g, text_emb_l, text_emb_g, sents, gpt_emb_l, gpt_emb_g, gpt_sents):

        ## contractive vision and bert 
        ...
        ## contractive vision and gpt 
        l_loss2, l_loss3, attn_maps2 = self._calc_local_loss(
            img_emb_l, gpt_emb_l, gpt_sents, is_GPT=True
        )
        g_loss2, g_loss3 = self._calc_global_loss(img_emb_g, gpt_emb_g)


        #  ## contractive gpt and bert 
        # l_loss4, l_loss5, attn_maps4 = self._calc_local_loss(
        #     gpt_emb_l, text_emb_l, sents
        # )
        g_loss4, g_loss5 = self._calc_global_loss(gpt_emb_g, text_emb_g)
        ...
        loss += (l_loss2 + l_loss3) * self.local_loss_weight
        loss += (g_loss0 + g_loss1) * self.global_loss_weight
        loss += (g_loss2 + g_loss3) * self.global_loss_weight
        loss += (g_loss4 + g_loss5) * self.global_loss_weight
    ...
    def forward(self, x):
        ...
        # GPT decoder branch
        gpt_emb_l, gpt_emb_g = self.gpt_decoder_forward(x["embeddings"])
        gpt_sents = x['emb_lens']

        return img_emb_l, img_emb_g, text_emb_l, text_emb_g, sents, gpt_emb_l, gpt_emb_g, gpt_sents
