...
class BertEncoder(nn.Module):
    ...
    def forward(self, ids, attn_mask, token_type):
        ...
        # print(f"Time taken for aggerate forward pass: {time.time() - s}")

        return word_embeddings, sent_embeddings, sents
    


class BertEncoderWithProj(nn.Module):
    def __init__(self, cfg):
        super(BertEncoderWithProj, self).__init__()
        ...
        #     self.bert_type, output_hidden_states=True,
        # )
        self.model =  BertModel.from_pretrained(self.bert_type, output_hidden_states=True, trust_remote_code=True)

        # self.tokenizer = AutoTokenizer.from_pretrained(self.bert_type)
        self.tokenizer = BertTokenizer.from_pretrained(self.bert_type, trust_remote_code=True)
        self.idxtoword = {v: k for k, v in self.tokenizer.get_vocab().items()}

        self.emb_global = nn.Linear(self.embedding_dim , self.embedding_dim)
        # self.emb_global = nn.Identity()
        self.emb_local = nn.Linear(self.embedding_dim , self.embedding_dim)
    ...
    def forward(self, ids, attn_mask, token_type):
        ...
        word_embeddings = word_embeddings.view(batch_dim * num_words, feat_dim)
        if self.emb_local is not None:
            word_embeddings = self.emb_local(word_embeddings)    # change local to global
        word_embeddings = word_embeddings.view(batch_dim, num_words, self.embedding_dim)
        word_embeddings = word_embeddings.permute(0, 2, 1)
...
class BertEncoderWithProjAtten(nn.Module):
    def __init__(self, cfg):
        super(BertEncoderWithProjAtten, self).__init__()
        ...
        self.model = AutoModel.from_pretrained(
            self.bert_type, output_hidden_states=True, output_attentions=True
        )
    ...
    def forward(self, ids, attn_mask, token_type):
        ...
        if self.last_n_layers > 1:
            ...
            # 计算多个层的 attention map，取最后 n 层的 attention
            all_attention_maps = outputs[3][-self.last_n_layers:]  # 取最后 n 层的注意力
            attention_map = torch.stack(all_attention_maps).mean(dim=0).mean(dim=1)  # 平均不同层和注意力头
        # use last layer
        else:
            word_embeddings, sent_embeddings = outputs[0], outputs[1]
            attention_map = outputs[3][-1].mean(dim=1)

        batch_dim, num_words, feat_dim = word_embeddings.shape
        word_embeddings = word_embeddings.view(batch_dim * num_words, feat_dim)
        if self.emb_global is not None:
            word_embeddings = self.emb_global(word_embeddings)
        word_embeddings = word_embeddings.view(batch_dim, num_words, self.embedding_dim)
        word_embeddings = word_embeddings.permute(0, 2, 1)
        ...
        return word_embeddings, sent_embeddings, sents, attention_map
    



class BertEncoderSimple(nn.Module):
    def __init__(self, cfg):
        super(BertEncoderSimple, self).__init__()
