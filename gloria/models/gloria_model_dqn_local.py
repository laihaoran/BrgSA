...
class GLoRIADQNLOCAL(nn.Module):
    def __init__(self, cfg):
        super(GLoRIADQNLOCAL, self).__init__()

        self.cfg = cfg
        self.text_encoder = builder.build_text_model(cfg)
        self.img_encoder = builder.build_img_model(cfg)
        self.fusion_module = builder.build_dqn_local_module(cfg)
        ...
        self.attention_fn = loss.gloria_loss.attention_fn
        self.attention_fn_T = loss.gloria_loss.attention_fn_T
    ...
    def forward(self, x):
        ...
        img_l = img_emb_l.view(img_emb_l.size(0), img_emb_l.size(1), -1).permute(0, 2, 1)  #(batch_size,patch_num,dim)
        text_l = text_emb_l.permute(0, 2, 1)  #(batch_size,query_num,dim)
  
        atten_plus_img = self.fusion_module(img_l, text_l, dim_reduce=False)
    
        i2t_cls = self.fusion_module(atten_plus_img.permute(1, 0, 2), img_emb_g, dim_reduce=True).squeeze(-1)

        atten_plus_text = self.fusion_module(text_l, img_l, dim_reduce=False)
    
        t2i_cls = self.fusion_module(atten_plus_text.permute(1, 0, 2), text_emb_g, dim_reduce=True).squeeze(-1)
        # i2t_cls = self.text2img(img_emb_l, text_emb_l, x["cap_lens"], agg='sum')

        # t2i_cls = self.image2text(text_emb_l, img_emb_l, x["cap_lens"], agg='sum')
        # 
        return img_emb_l, img_emb_g, text_emb_l, text_emb_g, sents, i2t_cls, t2i_cls
    
    def image2text(self, words_emb, img_features, cap_lens, agg):  
        batch_size = words_emb.shape[0]
        att_maps = []
        query = []
        context = words_emb.permute(0, 2, 1) 
        for i in range(img_features.shape[0]):
            patch = img_features[i, :, :, :].unsqueeze(0).contiguous()  # [768, 19, 19]
            patch = patch.repeat(batch_size, 1, 1, 1)  # [48, 768, 25]
            weiContext, attn = self.attention_fn_T(
                patch, words_emb, self.temp1
            )  # [48, 768, 25], [48, 25, 19, 19]
            weiContext = weiContext.permute(0, 2, 1)
            # row_sim = self.fusion_module(weiContext, word)  # use text as query to search img
            # row_sim = row_sim.view(batch_size, words_num).squeeze(-1)  # [48, 25]
            if agg == "sum":
                weiContext = weiContext.sum(dim=1, keepdim=True)  # [48, 1]
            else:
                weiContext = weiContext.mean(dim=1, keepdim=True)  # [48, 1]
            query.append(weiContext)
        query = torch.cat(query, 1)  #
        # context = img_features.view(img_features.size(0), img_features.size(1), -1)  # [48, 768, 19, 19]
        # context = context.permute(0, 2, 1)
        context = img_features.view(img_features.size(0), img_features.size(1), -1).permute(0, 2, 1)
        similarities =  self.fusion_module(context, query).squeeze(-1)
        # similarities1 = similarities.transpose(0, 1)  # [48, 48]
        return similarities

    def text2img(self, img_features, words_emb, cap_lens, agg): 
        ...
        for i in range(words_emb.shape[0]):
            ...
            ## use word and context to generate weiContext as image query
            weiContext, attn = self.attention_fn(
                word, context, self.temp1
            )  # [48, 768, 25], [48, 25, 19, 19]
            weiContext = weiContext.permute(0, 2, 1)
            # row_sim = self.fusion_module(weiContext, word)  # use text as query to search img
            # row_sim = row_sim.view(batch_size, words_num).squeeze(-1)  # [48, 25]
